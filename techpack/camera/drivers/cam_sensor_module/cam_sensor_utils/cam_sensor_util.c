...
static DEFINE_MUTEX(powernum_mutex);
		...
		struct cam_hw_soc_info *soc_info)
{
	...
	for (index = 0; index < ctrl->power_setting_size; index++) {
		...
		case SENSOR_CUSTOM_GPIO2:
			...
			if (power_setting->seq_type == SENSOR_CUSTOM_GPIO1) {
				mutex_lock(&powernum_mutex);
				++custom_gpio1_powernum;
				mutex_unlock(&powernum_mutex);
			}
	...
	for (index = 0; index < ctrl->power_down_setting_size; index++) {
		...
		case SENSOR_CUSTOM_GPIO2:
			...
			if (pd->seq_type == SENSOR_CUSTOM_GPIO1) {
				mutex_lock(&powernum_mutex);
				if (--custom_gpio1_powernum) {
					CAM_DBG(CAM_SENSOR,
						"custom_gpio1_powernum is %d, continue",
						custom_gpio1_powernum);
					mutex_unlock(&powernum_mutex);
					continue;
				}
				mutex_unlock(&powernum_mutex);
				CAM_DBG(CAM_SENSOR,
					"custom_gpio1_powernum is 0, power down");
			}
